...
class Model:
    ...
    def __init__(self):
        ...
        self.gamma = 0.5
        self.teams_n = 2
        self.features_n = 8 * self.teams_n + 1
    ...
    def __update_pi_rating(self, match: pd.Series):
        ...
        error = abs(expected_diff - real_diff)
        ps = 3 * log(1 + error, 10)  # c = 3, b = 10
        ps_h = ps if expected_diff < real_diff else -ps
        ps_a = ps if expected_diff > real_diff else -ps

        rah_hat = self.team_pi_rating[team_h]['H RTG'] + ps_h * self.lr
        self.team_pi_rating[team_h]['A RTG'] += (rah_hat - self.team_pi_rating[team_h]['H RTG']) * self.gamma
        self.team_pi_rating[team_h]['H RTG'] = rah_hat

        rba_hat = self.team_pi_rating[team_a]['A RTG'] + ps_a * self.lr
        self.team_pi_rating[team_a]['H RTG'] += (rba_hat - self.team_pi_rating[team_a]['A RTG']) * self.gamma
        self.team_pi_rating[team_a]['A RTG'] = rba_hat
    ...
    def get_features(self, team_h: int, team_a: int) -> np.ndarray:
        ...
        x_features[0] = self.team_historical_strength[team_h]['H WIN PCT']
        x_features[1] = self.team_historical_strength[team_h]['A WIN PCT']
        ...
        x_features[6] = self.team_pi_rating[team_h]['H RTG']
        x_features[7] = self.team_pi_rating[team_h]['A RTG']
        ...
        x_features[8] = self.team_historical_strength[team_a]['H WIN PCT']
        x_features[9] = self.team_historical_strength[team_a]['A WIN PCT']
        x_features[10] = self.team_historical_strength[team_a]['H GS AVG']
        x_features[11] = self.team_historical_strength[team_a]['A GS AVG']
        x_features[12] = self.team_historical_strength[team_a]['H GC AVG']
        x_features[13] = self.team_historical_strength[team_a]['A GC AVG']
        x_features[14] = self.team_pi_rating[team_a]['H RTG']
        x_features[15] = self.team_pi_rating[team_a]['A RTG']

        expected_diff = self.__expected_goal_diff(team_h, 'H RTG') - self.__expected_goal_diff(team_a, 'A RTG')
        x_features[16] = expected_diff
